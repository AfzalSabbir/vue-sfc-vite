 ...
 * @returns
 */
const createFiles = (options) => replaceable.filter((x) => x.language.includes(options.language)).forEach((replaceData) => createFile(options, replaceData));
/**
 *
 * @param {*} options
 * @param {*} replaceData
...
const createFile = (options, replaceData) => {
    ...
    fs.writeFileSync(distFile, ejs.render(replace, Object.assign(Object.assign({}, newOptions), replaces)));
};
export { createFiles };
